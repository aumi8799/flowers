...
<div class="container my-5">
   ...
   <div class="card p-4 mb-4 shadow-sm">
    ...
    @if($order->video && !$order->video_path)
            <form action="{{ route('order.uploadVideo', $order->id) }}" method="POST" enctype="multipart/form-data" class="d-inline-block me-2">
                @csrf
                <input type="file" name="video_file" accept="video/mp4" class="form-control mb-2" required>
                <button type="submit" class="btn btn-primary"><i class="fas fa-upload me-1"></i> Įkelti vaizdo įrašą</button>
            </form>
        @endif
    {{-- Grįžimas ir valdymas --}}
    <div class="d-flex justify-content-between mt-4">
        <a href="{{ route('courier.tasks') }}" class="btn btn-secondary">
            <i class="fas fa-arrow-left me-2"></i> Grįžti
        </a>


        @if($order->status != 'pristatytas')
            <form action="{{ route('order.delivered', $order->id) }}" method="POST" class="d-inline-block">
                @csrf
                @method('PUT')
                <button type="submit" class="btn btn-success"
                    @if($order->video && !$order->video_path) disabled @endif>
                    <i class="fas fa-check me-1"></i> Pažymėti kaip pristatytą
                </button>
            </form>
        @endif
    </div>
