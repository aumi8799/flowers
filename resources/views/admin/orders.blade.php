                    ...
                    @foreach($orders as $order)
                        <div class="col-md-12 mb-4">
                            <div class="card-glass p-4">
                                ...
                                <h5>Užsakymo ID: <a href="{{ route('admin.orders.show', $order->id) }}" class="text-success">#{{ $order->id }}</a></h5>
                                <h5>Pirkėjo paskyra: <span class="text-success">{{ $order->user->name ?? 'Nežinomas' }}</span></h5>
                                ...
                                @if ($order->cancel_reason)
                                    <p class="mb-1"><strong>Atšaukimo priežastis:</strong> {{ $order->cancel_reason }}</p>
                                @endif

                                <p class="mb-2"><strong>Užsakymo kaina:</strong> {{ $order->total_price }} €</p>

                                <a href="{{ route('admin.orders.show', $order->id) }}" class="btn btn-outline-success btn-sm">Peržiūrėti detales</a>
                            </div>
                        </div>
                    @endforeach
                </div>
